...
async def start(client, message: Message):
    ...
    if user_id in PREMIUM_USERS:
        await message.reply("🎉 **Welcome back!**\nYou are already a **Premium User**. Send a Terabox link to start downloading.")
        return
...
# When user clicks "Check Again" after joining channels
...
# Handle Terabox link and download videos
@bot.on_message(filters.text & ~filters.command("start"))
async def handle_link(client, message: Message):
    user_id = message.from_user.id
    link = message.text.strip()

    # If the user isn't marked as premium yet
    ...
    # If the link is a valid Terabox link
    if "terabox" in link:
        ...
        await message.reply("❌ Invalid Terabox link. Please send a valid link.")
